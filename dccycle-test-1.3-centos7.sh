...
# Based on "reboot-test-1.3-centos7.sh" to modify the script
# Sinper Laing, 2016/04/14
##################################################################################################
# 
...
#
# PS - this DC cycle script was for CentOS 7.1, it may got something wrong in CentOS 6.7.
#
#--- Changed history---------------------------------------------------------------
# 4/14: change to 1.3
...
# 
# 3.) add "sh /PATH/dccycle-test-1.1-centos7.sh" in root's .bashrc
# 	#vi /root/.bashrc
#	add "sh /PATH/dccycle-test-1.1-centos7.sh" in .bashrc file.
#
...
#
DCCYCLE_TEST_FOLDER=/var/log/dccycle-test
DCCYCLE_TEST_FILE=/root/dccycle-test-1.3-centos7.sh
#
trap sig_handler 15
...
function create_folder {
	...
	if [ -d "$DCCYCLE_TEST_FOLDER" ]; then
		whiptail --msgbox "The folder already exist! $DCCYCLE_TEST_FOLDER" 30 70
	else
		`mkdir "$DCCYCLE_TEST_FOLDER"`
		`chmod 755 "$DCCYCLE_TEST_FOLDER"`
	fi

	#export $DCCYCLE_TEST_FOLDER
	export PCI_info_file="$DCCYCLE_TEST_FOLDER"/pciinfo
	export CPU_info_file="$DCCYCLE_TEST_FOLDER"/cpuinfo
	export MEM_info_file="$DCCYCLE_TEST_FOLDER"/meminfo
	export HDD_info_file="$DCCYCLE_TEST_FOLDER"/hddinfo
	export USB_info_file="$DCCYCLE_TEST_FOLDER"/usbinfo
	export SMBIOS_file="$DCCYCLE_TEST_FOLDER"/smbios
	export SEL_detail="$DCCYCLE_TEST_FOLDER"/sel-detail
	export DateTime_file="$DCCYCLE_TEST_FOLDER"/datetime
	export DMESG_file="$DCCYCLE_TEST_FOLDER"/dmesg
	#export FRU_file="$DCCYCLE_TEST_FOLDER"/fru-table
	export TARGET_times_file="$DCCYCLE_TEST_FOLDER"/target_times
	export COUNTER_file="$DCCYCLE_TEST_FOLDER"/counter
	export WAIT_time_file="$DCCYCLE_TEST_FOLDER"/wait_time

}	


function dccycle_init {
	...
	if [ -d "$DCCYCLE_TEST_FOLDER" ]; then
		...
		WAIT_TIME=$(whiptail --backtitle "Reboot test script init" --title "Set waiting time(seconds)." --inputbox "Enter every time to wait SUT dccycle.(seconds):" 30 70 30 --title "Wait time for every cycle" 3>&1 1>&2 2>&3)
		...
		lscpu | sed '15,16d' > "$DCCYCLE_TEST_FOLDER"/cpuinfo
		free | grep Mem | awk '{print $2}' > "$DCCYCLE_TEST_FOLDER"/meminfo
		lsscsi > "$DCCYCLE_TEST_FOLDER"/hddinfo
		lsusb > "$DCCYCLE_TEST_FOLDER"/usbinfo
		lspci > "$DCCYCLE_TEST_FOLDER"/pciinfo
		dmidecode > "$DCCYCLE_TEST_FOLDER"/smbios
		ipmitool sel elist > "$DCCYCLE_TEST_FOLDER"/sel-detail
		#ipmitool fru > "$DCCYCLE_TEST_FOLDER"/fru-table
		...
		echo "$DCCYCLE_TEST_FILE" >> /root/.bashrc
		
		whiptail --backtitle "Reboot test script init" --title "Check files" --msgbox "\n
		Check the files in "$DCCYCLE_TEST_FOLDER": \n
		...
		if [ $? = 0 ] ; then
			...
			#dccycle here for first.
			exec ipmitool chassis power cycle
		else
		...
		whiptail --msgbox "The log folder Not found, "$DCCYCLE_TEST_FOLDER"." 30 70
...
if [ -d "$DCCYCLE_TEST_FOLDER" ]; then	
	...
	PCI_info_file="$DCCYCLE_TEST_FOLDER"/pciinfo
	CPU_info_file="$DCCYCLE_TEST_FOLDER"/cpuinfo
	MEM_info_file="$DCCYCLE_TEST_FOLDER"/meminfo
	HDD_info_file="$DCCYCLE_TEST_FOLDER"/hddinfo
	USB_info_file="$DCCYCLE_TEST_FOLDER"/usbinfo
	SMBIOS_file="$DCCYCLE_TEST_FOLDER"/smbios
	SEL_detail="$DCCYCLE_TEST_FOLDER"/sel-detail
	DateTime_file="$DCCYCLE_TEST_FOLDER"/datetime
	DMESG_file="$DCCYCLE_TEST_FOLDER"/dmesg
	#FRU_file="$DCCYCLE_TEST_FOLDER"/fru-table
	TARGET_times_file="$DCCYCLE_TEST_FOLDER"/target_times
	COUNTER_file="$DCCYCLE_TEST_FOLDER"/counter
	WAIT_time_file="$DCCYCLE_TEST_FOLDER"/wait_time
	...
	if [ "$COUNTER" -gt "$TARGET_RUN" ]; then
		...
		Date=`date +%F`
		`tar cfzk dccycle-test-"$Date".tar.gz "$DCCYCLE_TEST_FOLDER"`
		lslog=`ls *.tar.gz`

		# remove the test folder
		rm -fr "$DCCYCLE_TEST_FOLDER"

		if [ -f dccycle-test-"$Date".tar.gz ]; then
			whiptail --backtitle "Test completed" --title "Testing finished." --infobox " The testing was finished.\n
				...
				The test log files was located: $DCCYCLE_TEST_FOLDER \n
				---------------------------------------------------\n
				backup the log folder: please check the file like dccycle-test-DATE.tar.gz \n
				If you plan to run another dccycle test, please remove whole test log folder: 
				$DCCYCLE_TEST_FOLDER \n
				" 30 100
			exit
		else
...
#
	...
	#	`ipmitool fru > "$FRU_file"-"$COUNTER"`
		...
		`ipmitool sel elist >> $DCCYCLE_TEST_FOLDER/sel-summary`
		...
		if [ -z "$SEL_ERR" ]; then
			...
			echo "$SEL_ERR" > "$DCCYCLE_TEST_FOLDER"/err-sel-"$COUNTER"
			err_sel_show=`awk '{print $7" "$8}' "$DCCYCLE_TEST_FOLDER"/err-sel-"$COUNTER" `
...
#		fi
		...
		dialog --pause "Waiting "$WAIT_TIME" second to dccycle..\n\n
		----------------------------------------------------\n
		Product: "$Product"\n
		BIOs   : "$BiosVersion" \n
		BMC    : "$BmcVersion" \n\n
		...
		\n\n\n\n\n
		====================================================\n
		Press OK to dccycle now, press cancel to stop the script.
		" 50 100 "$WAIT_TIME"
